...
export default function ContactForm() {
  ...
  const proceedWithDiscuss = (msg) => {
    ...
    fetchMessage({ messages: [{ content: msg, role: MESSAGE_ROLE_USER }] })
      .then(async (res) => {
        ...
        const toolContact = toolCalls.find((tool) => isStrEqualCi(tool.function.name, MODE_CONTACT));
  ...
  const validateMessage = async (msgContent) => {
    ...
    if (mode === MODE_CONTACT) {
      ...
      if (isStrEqualCi(msgContent, CMD_CANCEL)) {
    ...
    if (isStrEqualCi(msgContent, MODE_CONTACT)) {
      toggleContact();
    } else {
  ...
  return (
    ...
    >
      <ul className="p-4 text-sm flex flex-col gap-1">
        <ChatMessage className="bg-linear-to-tr bg-primary-700">
          <p>Bonjour!</p>
            ...
            <ChatMessage
              className={
                message.role === MESSAGE_ROLE_USER
                  ? 'bg-linear-to-tr from-blue-200 to-blue-300 dark:from-blue-400 dark:to-blue-500'
                  : message.isError
                    ? 'bg-linear-to-tl from-red-200 to-red-300 dark:from-red-400 dark:to-red-500'
                    : 'bg-linear-to-tr bg-primary-700'
              }
